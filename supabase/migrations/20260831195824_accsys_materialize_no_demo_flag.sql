-- the platform allows only one is_demo organization and it is already taken,
-- so the migration sandbox is a normal TRIAL org identified by its slug.
...
begin
  insert into public.organizations (name, slug, status, default_currency, entity_type, governorate)
  select 'اتحاد شاغلين قرية باجوش — ترحيل AccSys (بيئة اختبار)', 'accsys-migration-demo',
         'TRIAL', 'EGP', 'OWNERS_ASSOCIATION', 'مطروح'
