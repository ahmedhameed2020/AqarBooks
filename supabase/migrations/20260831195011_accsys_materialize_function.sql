...
declare v_org uuid; v_prop uuid; v_rc uuid;
begin
  -- 1. tenant -------------------------------------------------------------
  insert into public.organizations (name, slug, status, default_currency, entity_type,
                                    governorate, country_code_placeholder, is_demo)
  select 'اتحاد شاغلين قرية باجوش — ترحيل AccSys (تجريبي)', 'accsys-migration-demo',
         'TRIAL', 'EGP', 'OWNERS_ASSOCIATION', 'مطروح', null, true
  ...
  -- 2. chart of accounts, level by level so parent_id always resolves ------
  for step in select 'coa' loop end loop;
  ...
  -- 4. members and ownerships ---------------------------------------------
  insert into public.members (organization_id, full_name, is_company, customer_type, legal_name)
  select v_org, m.full_name, m.is_company, case when m.is_company then 'B2B' else 'B2C' end, m.full_name_en
  from accsys_stage.member m
  where not exists (select 1 from public.members x
                    where x.organization_id = v_org and x.legal_name = m.full_name_en
                      and x.full_name = m.full_name);
  insert into accsys_stage.member_map (acc_code, member_id)
  select m.acc_code, x.id from accsys_stage.member m
  join lateral (select id from public.members y
                 where y.organization_id = v_org and y.full_name = m.full_name
                   and y.legal_name = m.full_name_en limit 1) x on true
  ...
  step := 'coa marked used'; get diagnostics n = row_count; return next;
  return;
end $fn$;;
