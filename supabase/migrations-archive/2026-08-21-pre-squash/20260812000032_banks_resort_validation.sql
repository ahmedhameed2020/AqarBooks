-- Closes the cross-resort mixing gap identified in the /finance/banks
-- audit: neither record_incoming_cheque nor record_payment (the 12-arg
-- cashier-session overload defined in 20260810000030, the one
-- clear_incoming_cheque actually calls -- confirmed by argument count/types
-- before writing this, since this schema has bitten itself before with
-- accidental overloads) validated that p_resort_id belongs to
-- p_organization_id, or that an allocated due's own resort_id matches the
-- resort the payment/cheque is being recorded against. A cheque banked
-- into Resort A's account could silently settle a due belonging to
-- Resort B within the same organization. Same argument list as the
-- functions being replaced, so CREATE OR REPLACE is a real replacement,
-- not a new overload.
...
begin
  ...
  select resort_id into v_account_resort_id
  from public.bank_accounts
  where id = p_bank_account_id and organization_id = p_organization_id;
  if v_account_resort_id is null then
    raise exception 'BANK_ACCOUNT_NOT_IN_ORGANIZATION: الحساب البنكي المحدد لا يتبع لهذا الكيان' using errcode = '22023';
  end if;
  if v_account_resort_id <> p_resort_id then
    raise exception 'BANK_ACCOUNT_RESORT_MISMATCH: الحساب البنكي المحدد يتبع موقعًا مختلفًا عن الموقع المحدد' using errcode = '22023';
  end if;
...
-- record_payment (12-arg cashier-session overload -- returns uuid). Adds
-- RESORT_REQUIRED/RESORT_NOT_IN_ORGANIZATION (same pattern used across the
-- suppliers/expenses modules) plus, inside the existing allocation loop,
-- a check that every allocated due's own resort_id matches p_resort_id --
-- the specific gap that let a cheque settle another resort's receivable.
