-- Code review follow-up on Task 3 (post_payment_internal extraction):
-- post_payment_internal was created with `set search_path = public, pg_temp`,
-- diverging from the fixed `search_path = public` (or `public, extensions`
-- where pgcrypto is needed) convention used by every other SECURITY DEFINER
-- function in this codebase (40+, including record_payment itself). That
-- convention is the standard defense against search_path-injection attacks
-- on SECURITY DEFINER functions -- adding pg_temp back onto the path
-- reopens exactly the attack surface it exists to close, and here it buys
-- nothing: every identifier in this function's body is already
-- schema-qualified (public.dues, public.payments, public.cashier_sessions,
-- etc.), so there was never an actual defense-in-depth benefit to the
-- wider path. Fixing at the source before Task 4's record_online_payment
-- copies the same pattern.
--
-- Also folds in the reviewer's Minor suggestion: record_payment's call
-- into post_payment_internal now uses named-parameter syntax instead of
-- positional, so a future reordering of post_payment_internal's parameter
-- list can't silently mis-wire this call site.
...
-- create or replace preserves the prior grant state (owner-only execute,
-- no PUBLIC/authenticated/anon grants) -- no revoke statements needed here.

-- record_payment: same body, only the call site into post_payment_internal
-- changes from positional to named-parameter arguments.
