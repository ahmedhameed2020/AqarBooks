...
export async function startDemoSession(): Promise<DemoStartResult> {
  ...
  // 3. The organization must ALSO be flagged in the database. The env var says
  //    which tenant the public door opens; organizations.is_demo is what the
  //    RLS write policies and the lease-rent sweep actually key off. If the two
  //    disagree, this is the direction that matters: a tenant opened as a demo
  //    but not flagged as one would receive visitors with none of the database
  //    write protections applied to it. Read through the demo session's own
  //    client, so the row is subject to the same RLS as everything else it sees.
  ...
  if (orgError || orgRow?.is_demo !== true) {
    await supabase.auth.signOut();
    console.error(
      "[demo] refusing entry: DEMO_ORGANIZATION_ID names an organization that is not flagged is_demo",
    );
    return { ok: false, error: "demo_misconfigured" };
  }
